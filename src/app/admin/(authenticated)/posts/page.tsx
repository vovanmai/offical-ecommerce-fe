...
import { list as listPosts, deletePost } from '@/api/admin/post'
...
const Page = () => {
  ...
  const actions = (
    <Link href={ ADMIN_ROUTES.POST_CREATE }>
      <Button
        size="large"
        type="primary"
      >
  ...
  const fetchData = async (params = {}) => {
    setLoading(true);
    try {
      const response = await listPosts(removeEmptyFields(params));
  ...
  const handleTableChange: TableProps<any>['onChange'] = async (pagination, filters, sorter) => {
    ...
    router.push(`/admin/posts?${queryString}`)
  }
  ...
  const columns: ColumnsType<any> = [
    ...
    },
    {
      ...
      render: (text, record) => {
        return (
          <Link style={{ color: colorPrimary }} href={`/admin/posts/${record.id}/edit`}>{text}</Link>
        )
      }
    },
    {
    ...
    },
    {
      ...
      render: (record) => {
        return (
          <Link style={{ color: colorPrimary }} href={`/admin/post-categories/${record.category_id}/edit`}>{ record.category_name }</Link>
        )
      }
    },
    {
  ...
  return (
    <div>
      <Breadcrumb items={[{title: 'Bài viết'}]} />
      <ConfirmModal
      ...
      />
      <Card title="Danh sách bài viết" variant="outlined" extra={actions}>
        <Search
          onSearch={onSearch}
